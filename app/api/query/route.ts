...
import { answerQueryRequest } from '@/lib/queryService';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await answerQueryRequest(parsed.value);
    ...
    return NextResponse.json({ error: `Query processing failed: ${message}` }, { status: 500 });
  }
}
